# -*- coding: utf-8 -*-
"""
Created on Sat Jul 30 16:26:51 2022
...
temp = df_agg_diff.iloc[:,numeric_cols]
temp = (temp - median_agg).div(median_agg)
...
views_days = pd.pivot_table(df_time_diff_yr,index= ['days_published'],values =['Views'], aggfunc ={'days_published':[np.mean,np.median,lambda x: np.percentile(x, 80),lambda x: np.percentile(x, 20)]}).reset_index()
...
if add_sidebar == 'Aggregate Metrics':
    ...
    st.dataframe(df_agg_diff_final.style.hide_index().applymap(style_negative, props='color:red;').applymap(style_positive, props='color:green;').format(df_to_pct))
